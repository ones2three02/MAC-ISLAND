...
final class AppModel {
    ...
    func forceKillSessionProcess(sessionID: String) {
        guard let session = state.session(id: sessionID), let pid = session.pid else {
            return
        }
        monitoring.forceKillProcess(pid: pid)
    }
    func repairHooks() {
        Task { @MainActor in
    ...
    init(
        ...
        scheduler.onActiveModuleChanged = { [weak self] in
            self?.refreshOverlayPlacementIfVisible()
        }
        // 注册默认模块
        scheduler.registerModule(AgentMonitorModule(model: self))
        scheduler.registerModule(SystemTelemetryModule())
    ...
    func showSettings() {
        ...
        if let window = NSApp.windows.first(where: { $0.title == "Mac Island Settings" }) {
    ...
    func approvePermission(for sessionID: String, action: ApprovalAction) {
        ...
        switch action {
        case .deny:
            resolution = .deny(message: "Permission denied in Mac Island.", interrupt: false)
    ...
    private func permissionResolution(for approved: Bool) -> PermissionResolution {
        ...
        return .deny(message: "Permission denied in Mac Island.", interrupt: false)
    }
