...
struct V6ClosedPill: View {
    ...
    var leftCustomView: AnyView? = nil
    var rightCustomView: AnyView? = nil
    var leftCustomViewWidth: CGFloat = 24
    var rightCustomViewWidth: CGFloat = 0
    ...
    private var externalBody: some View {
        let glyphW: CGFloat = leftCustomViewWidth
        let labelW = label.map { V6CenterLabelView.intrinsicWidth(of: $0) } ?? 0
        let rightW = rightCustomView != nil ? rightCustomViewWidth : (rightSlot.map { V6RightSlotView.intrinsicWidth(of: $0) } ?? 0)

        let labelBlock = (label == nil ? 0 : 6 + labelW)
        let rightBlock = ((rightSlot == nil && rightCustomView == nil) ? 0 : Self.innerGap + rightW)
        ...
        return ZStack {
            ...
            HStack(spacing: 0) {
                ...
                if let rightCustomView {
                    rightCustomView
                } else if let rightSlot {
