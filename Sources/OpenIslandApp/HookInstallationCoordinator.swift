...
final class HookInstallationCoordinator {
    ...
    var claudeUsageStatusSummary: String {
        ...
        if status.managedStatusLineNeedsRepair {
            return "Mac Island detected a missing managed Claude status line script and will repair it automatically."
        }

        if status.hasConflictingStatusLine {
            return "Mac Island will not overwrite an existing Claude status line automatically."
        }
