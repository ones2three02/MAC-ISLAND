...
if [[ "$(uname -s)" != "Darwin" ]]; then
    echo "Mac Island packaging runs only on macOS." >&2
    exit 1
fi

repo_root="$(cd "$(dirname "$0")/.." && pwd)"
app_name="${OPEN_ISLAND_APP_NAME:-Mac Island}"
bundle_identifier="${OPEN_ISLAND_BUNDLE_ID:-app.openisland.OpenIsland}"
