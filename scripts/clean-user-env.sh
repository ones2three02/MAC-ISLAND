...
#
# This removes all Mac Island (and legacy Vibe Island) artifacts from the
...
echo "==> Quit Mac Island if running"
if ! $DRY_RUN; then
...
echo ""
echo "==> Cleaning Mac Island artifacts"

# --- Hook configurations ---
echo "--- Hook configs ---"

# Claude-style forks (.claude / .qoder / .qwen / .factory / .codebuddy / .gemini):
# each has a settings.json that may contain Mac Island hook entries, plus
...
# Codex: remove Mac Island entries from hooks.json
...
if $DRY_RUN; then
    ...
    echo ""
    echo "Next steps:"
    echo "  1. Install Mac Island.dmg from the latest release"
