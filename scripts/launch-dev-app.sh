...
bundle_dir="$HOME/Applications/Mac Island Dev.app"
...
osascript -e 'tell application "Mac Island Dev" to quit' 2>/dev/null || true
pkill -9 -f "Mac Island Dev" 2>/dev/null || true
sleep 2
...
<dict>
    ...
    <string>Mac Island needs automation access to focus Terminal and iTerm sessions for jump-back.</string>
...
if security find-identity -p codesigning -v "$HOME/Library/Keychains/login.keychain-db" 2>/dev/null \
       | grep -q '"Mac Island Dev Local"'; then
    sign_identity="Mac Island Dev Local"
else
    echo
