...
### Phase 2 — Jump (when user clicks jump in Mac Island)
...
`CGEventPost` requires macOS Accessibility permission for the parent process (Mac Island). On first jump attempt, if `AXIsProcessTrustedWithOptions` returns false:

- Show a one-time in-app prompt explaining why the permission is needed ("so Mac Island can focus the exact Warp tab for your agent session")
...
4. Click jump on the 2nd session in the Mac Island session list while the 4th session is currently focused in Warp.
...
1. **Upstream feature request to Warp** — draft and file a GitHub issue requesting a `warp://action/focus_cli_agent?session_id=…` URL scheme action. Point out that `CLIAgentSessionsModel` already tracks every session internally, and a public entry point would allow any external integration (not just Mac Island) to do one-click precision jump without scraping SQLite. Reference the `claude-code-warp` plugin's current one-way integration as the motivating use case.
...
4. **Settings for cycle keystroke** — expose a text field in Mac Island settings for the cycle keystroke, default `Cmd+Shift+]`.
