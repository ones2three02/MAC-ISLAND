...
**Goal:** When a user clicks jump on a Warp-hosted Claude or Codex session in Mac Island, automatically focus the exact Warp tab running that session (one click, no manual tab scanning).
...
enum WarpSQLiteFixture {
    ...
    struct Scenario {
        ...
        /// tab 2 cwd=mac-island (pane uuid BBBB...), tab 3 cwd=/tmp (pane uuid CCCC...).
        /// Claude was launched in giftcard from Warp shell session 1001 and in mac-island from 1002.
        static let threeTabsTwoClaudes = Scenario(rows: [
            ...
            "INSERT INTO terminal_panes (id, uuid, cwd) VALUES " +
                ...
                "(2, x'BBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBB', '/Users/u/mac-island')," +
            ...
            "INSERT INTO commands (id, session_id, command, pwd) VALUES " +
                ...
                "(200, 1002, 'claude --dangerously-skip-permissions', '/Users/u/mac-island');",
...
func lookupPaneUUIDReturnsUppercaseHexForClaudeInKnownCwd() throws {
    ...
    let uuid = reader.lookupPaneUUID(forCwd: "/Users/u/mac-island")
...
func currentFocusedPaneUUIDReturnsActiveTabsUUID() throws {
    ...
    // Scenario has active_tab_index = 1 which is tab id 2 (mac-island).
    let reader = WarpSQLiteReader(databasePath: tmp)
...
If `/Applications/Mac Island.app` is running, quit it first (`osascript -e 'quit app "Mac Island"'`) so there's no bridge-socket collision.

- [ ] **Step 2: Grant Accessibility permission**

Open **System Settings → Privacy & Security → Accessibility**. If `OpenIslandApp` is listed, toggle it on. If not, trigger a jump attempt from Mac Island first (which will cause macOS to add it to the list) and come back.

- [ ] **Step 3: Set up the multi-tab scenario**

1. Open Warp.
2. Open 4 new tabs, each in a **different directory**:
   - `cd ~/Developer/mac-island && claude`
...
1. Open Mac Island's session list (notch / top-bar).
...
2. In Mac Island, click jump for that same session.
...
1. **No permission**: temporarily disable OpenIslandApp in System Settings → Accessibility. Click jump. Expected: Warp activates, no cycling, Mac Island toast/log says "Grant Accessibility permission to enable precision jump."
2. **Re-enable permission** and continue.
3. **Close a Claude tab** while the session is still in Mac Island's list. Click jump for the closed session. Expected: cycling runs the full cap, then returns "Activated Warp but could not confirm precision focus." — never crashes.
