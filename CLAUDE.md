...
Mac Island — native macOS companion for AI coding agents. Sits in the notch / top bar, monitors local sessions, surfaces permission and question events, and jumps back to the right terminal/IDE. Local-first, no server.
...
## Dev app (Mac Island Dev.app)

`~/Applications/Mac Island Dev.app` is a wrapper around the repo build, not a separate product.
...
- Title: `Mac Island vX.Y.Z — Short English Title`. Installation section bilingual.
