<p align="center">
  <img src="docs/images/readme-banner.png" alt="Mac Island - agents in your menu bar" width="760">
</p>
...
<p align="center">
  ...
  <br>
  开源、本地优先、原生 macOS 的 AI Coding Agent 伴侣与灵动刘海多功能助手（集成番茄钟、系统遥测与音乐控制）。
  <br><br>
  <strong>中文</strong> | <a href="README.md">English</a>
</p>

<p align="center">
  <a href="https://github.com/ones2three02/MAC-ISLAND/releases/latest"><img src="https://img.shields.io/github/v/release/ones2three02/MAC-ISLAND?style=flat-square&label=release&color=blue" alt="最新版本"></a>
...
<p align="center">
  <a href="https://github.com/ones2three02/MAC-ISLAND/releases">下载</a> ·
  <a href="#快速开始">快速开始</a> ·
...
Mac Island 驻留在 Mac 的**刘海区域**（或顶部栏），不仅为你的 AI coding agents 提供实时控制面板（会话状态、权限审批、一键跳回终端），更是集成了 **Antigravity/Codex 桌面端双向监控**、**专业级专注番茄钟**、**系统硬件状态监测与 IP 定位**、以及**高精度音乐控制器**的 macOS 原生灵动刘海助手。全程不打断你的工作流。

可以理解为开源增强版的 [Vibe Island](https://vibeisland.app/)——**免费、本地运行、代码完全属于你**。

> *You don't need to pay for a product you can vibe, since you are a vibe coder.*

## 核心亮点

- **开源与本地优先** — 基于 GPL v3 开源，无服务器、无遥测、无数据泄露，一切运行在你的 Mac 本地。
- **原生 macOS 体验** — 使用 SwiftUI + AppKit 纯原生开发（非 Electron 套壳），支持超凡的**亚克力毛玻璃 (Glassmorphism)** 视觉效果以及精致的物理按压缩放反馈。
- **AI Coding Agent 伴侣** — 统一管理 Claude Code、Codex、Cursor、Gemini CLI、Kimi CLI 等多款 Agent；支持一键精准跳回对应的终端（Ghostty, iTerm2, WezTerm, Terminal, Zellij, tmux 等）及 IDE 工作区。
- **Antigravity & Codex 桌面端双向感知** — 深度融合桌面级 Agent 工具，通过 `antigravity://` / `codex://` 深度链接实现一键拉起或对话级别跳转，并支持实时 Quota 额度追踪。
- **专业专注番茄钟** — 支持工作/休息多状态循环设定、高自由度时间调节、今日番茄个数统计，深度集成系统通知中心推送。
- **系统硬件遥测 & IP 盾** — 实时监控 CPU 状态、内存 (MEM) 占用环形百分比，并支持异步获取局域网/公网 IP 及地理位置，提供三重 Fallback 的 IP 盾定位能力。
- **第三方媒体控制器** — 深度兼容网易云音乐等主流播放器，提供超美专辑封面高斯模糊阴影、高精度微交互面板（支持快进、播放/暂停、曲目切换及防后台夺焦锁定）。
...
| 毛玻璃 UI (Glassmorphism) | 惊艳的亚克力模糊面板风格，搭配精致物理按压交互反馈 |
| 专业专注番茄钟 | 深度集成工作/休息多模式切换、今日专注番茄统计和 macOS 系统通知中心 |
| 系统监控与定位 | 实时仪表盘，监测 CPU、内存（MEM）占用环形图，异步获取公网/局域网 IP 及地理位置 |
| 媒体播放器控制 | 适配第三方媒体播放器状态，高精度专辑封面渲染与快捷切歌/暂停操作 |
| 设置 | Hook 安装/卸载、用量仪表盘 |
...
从 [GitHub Releases](https://github.com/ones2three02/MAC-ISLAND/releases) 下载最新 DMG——已签名公证，开箱即用。
...
首次启动时，Mac Island 会自动发现活跃的 agent 会话并启动 live bridge。Hook 安装在 app 内的**设置**窗口管理。
...
Hooks **fail open**——如果 Mac Island 没在运行，你的 agents 不受任何影响。
...
我们非常欢迎 Issue、Pull Request 以及任何形式的贡献。详见 [CONTRIBUTING.zh-CN.md](CONTRIBUTING.zh-CN.md)。
...
```
我在使用 Mac Island (https://github.com/ones2three02/MAC-ISLAND) 时遇到了问题。
...
1. 收集我的环境信息：
   ...
   - 检查 Mac Island 是否在运行：`ps aux | grep -i "open.island\|OpenIslandApp" | grep -v grep`
...
`Mac Island` 会在刘海区域或顶部栏放置一个轻量控制界面，让你可以在不中断当前 flow 的前提下观察 live coding agents、跟踪会话进度，并快速跳回正确的 terminal 上下文。
...
- **Kimi CLI** — 基于 hook 的集成，通过 `~/.kimi/config.toml` 的 `[[hooks]]` 数组配置（Moonshot AI）。Kimi 的 hook payload 与 Claude Code 字段兼容，Mac Island 复用 Claude 解码路径，仅新增了 TOML installer。订阅 `SessionStart`、`UserPromptSubmit`、`Stop`、`Notification`、`PreToolUse`、`PostToolUse`。需要 Kimi CLI Hooks Beta。使用 `--source kimi` 调用 hooks binary。可以从设置窗口管理安装，或通过 CLI：
...
该脚本会创建 `output/package/Mac Island.app` 和 `output/package/Mac Island.zip`。传入 `OPEN_ISLAND_SIGN_IDENTITY` 可以签名。详见 [docs/packaging.md](docs/packaging.md)。
